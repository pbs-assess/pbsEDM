...
save the movies to GitHub, so not doing that now in case they change again. Plus
some are saved in R/functions, I think just while developing. Move to here when finalised.

TODO: Could simplify some of this, just save a vector also as `Nt_example`
instead of using the tibble.

TODO: Change calc2 to something else, see other vignettes.
...
An animated movie (with simlex results) is shown later below; to create the movie
...
Such a movie is useful for checking any single prediction. For example, just
change tstar to 15 (see the technical aside above to make a new movie):
```{r EDMmovie2, eval = FALSE}
# ```{r EDMmovie, animation.hook = 'gifski', interval = 8}
plot_explain_edm_movie(calc2,
                       tstar = 15)  # 15 shows the idea nicely
```
