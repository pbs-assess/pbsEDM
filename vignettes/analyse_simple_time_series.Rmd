...
The tibble `Nx_lags_orig` is saved in the package and contains several columns,
though we are only going to use the original time series of simulated population numbers,
$N(t)$, which has column name `Nt`. See `?Nx_lags_orig` for full details, and
...
Use only the `Nt` column from `Nx_lags_orig` and apply EDM
...
plot(calc2,
     portrait = FALSE)
```
The figure shows, before doing EDM, the data in various ways: time series of $N_t$ and $X_t$, plus phase plots of $N_t$ vs $N_{t-1}$, $X_t$ vs $X_{t-1}$ (as used for $E=2$) and $X_t$ vs $X_{t-1}$ vs $X_{t-2}$ (as used for $E=3$). The last 3 values of $N_t$ are in red, with the final one as a star. In the $X_t$ time series panel, the values of $X_t$ are also plotted in a vertical line (to the left of $t10$) which is the one-dimensional equivalent of a phase plot for $E=1$.
...
where, for example $E=2$ uses first-difference values $X_t$ and $X_{t-1}$:
```{r Eres}
...
in the final panel, the predicted value of each data point, for different values of $E$:
```{r Eresplot, fig.width = 9, fig.height = 5.36}
plot_pbsEDM_Evec(E_results,
                 portrait = FALSE)
```
The value $\rho$ is the correlation coefficient (TODO check terminology) between
the observed and predicted values, interpreted as the forecast skill.

A movie of that is run using:
...
issues -- see
[here](https://htmlpreview.github.io/?https://raw.githubusercontent.com/andrew-edwards/sizeSpectra/master/doc/MEPS_IBTS_recommend.html)
