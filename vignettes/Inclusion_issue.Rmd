...
pbs_calc <- pbs_simplex_1(Nx_lags_orig,
                          col_name = "Xt")    # full output list
testthat::expect_equal(Nx_lags_orig$Xt,
                       pbs_calc$pred_tbl$obs$Xt)        # check the indexing is the same
...
#   1/100 mismatches
#   [100] -0.136 - NA == NA
testthat::expect_equal(Nx_lags_orig$my.pred[1:99], pbs_pred[1:99])   # This passes
Nx_lags_orig$my.pred[100]
pbs_pred[100]
length(pbs_pred)
```
So `pbs_pred` gives a different result for the final time step. Presumably it should
