...
TODO Still need to exclude invalid $t^*$. Starting fresh:
```{r again}
ssr_new <- state_space_reconstruction_for_sve(data = simulated[1:15, ],
                                              response = "R_t",
                                              lags = lags_use_multi)
ssr_new     # Has the correct NA's in top two rows and final row
...
# Since `ssr_new` contains the response variable, I think that
#`distances_new` is considering it, which we don't want. Check:

distances_new_drop_response <- state_space_distances_for_sve(ssr_new[, -1])

# YES, they are difference. Suggest adding response option to
# state_space_distances_for_sve() and dropping it from calculations. TODO TODO
# TODO. No, just strip reponse_scaled out as a vector from ssr. Or actually use
# ssr calc but just for response with lag of 0.

forecasts_new <- state_space_forecasts_for_sve(ssr_new,
                                               distances_new)

# TODO Then back to L111 in `state_space_forecasts_for_sve.R`

HERE:
   remove response from call to state_space_reconstruction_for_sve, then call it
   a second time with just the response variable and no lags.

i.e. this will work, without the duplicated column, and then (within calling
                                                              function) remove
                                                              the 0 from column name
response_s <- state_space_reconstruction_for_sve(data = simulated[1:15, ],
                                                 response = "R_t",  # this won't
                                                   # then be needed
                                                 lags = list(R_t = 0))
