...
Jump to Where we are at section.
...
# TODO not sure why this fails, might be the use of random numbers
#  expect_equal(EDMsimulate::salmon_sim()[1:15, ], EDMsimulate::salmon_sim(T = 15))

TODO HERE Redo with simulation with changing harvest rate. And change error in
`state_space_reconstruction_for_sve.R` to something more useful.

simulated_small_3 <- simulated[1:15, ]
ssr_new <- state_space_reconstruction_for_sve(data = simulated_small_3,
                                              lags = lags_use_multi)
ssr_new     # Has the correct NA's in top two rows and final row   TODO TODO BUT
            # REST IS WRONG, first two columnns are the same but should not
            # be. Except they can be for ours as harvest rate is constant.

response_s <- state_space_reconstruction_for_sve(data = simulated_small_3,
                                                 response_only = TRUE,
                                                 lags = list(R_t = 0))
...
```{r untransform}
N_observed <- simulated_small_3$"R_t"
...
# with browser, seems that ssr and response_s are the same but shifted, but one
# should be R_t_0 and one S_t_1. So I think ssr calculation is using the wrong
# thing.
# It's using the same column whether I do R_t or S_t: this should not be
# essentially zero
state_space_reconstruction_for_sve(data, lags = list(S_t = 1)) -
 state_space_reconstruction_for_sve(data, lags = list(R_t = 1))
# HERE. Selecting p_t column (arbitrary) does give a different
# answer, so it's not always picking the same column - SEE ABOVE, it's because
# of the simulations


mve_small <- multiview_embedding(data = simulated_small_3,
                                 response = "R_t",
                                 lags = lags_use_multi)
                                 # TODO orig function: when I had this as lags_use,
                                 # first six non-NA forecast values were
                                 # same as for lags_use_multi, but not the rest;
                                 # maybe think if that makes sense or not now.
