...
ssr_small_2 <- state_space_reconstruction_for_sve(data = simulated_small_2,
                                                   response = "R_t",
                                                   lags = lags_use_multi)
# Okay, in the function it puts the response variable first as "R_t", but if
# we've prescribed a lag of 0 for "R_t" you get an identical column but labelled
# "R_t_0":

# TODO I don't see the need for extra first column. Wait to hear back from Luke.

HERE - decide what to name the columns in the above
function. Can't stick with R_t if it has then been first-differenced and scaled (even without first differencing I've just realised it was confusing sticking with the same names, but I like the annotating of lag number afterwards, so keep that. Maybe Rs_t, Ss_t_0 etc. where s is scaled (having first-differenced).
head(ssr_small_2)
