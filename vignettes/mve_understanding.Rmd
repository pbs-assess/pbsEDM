...
TODO maybe. Had thought `max_lag` in `untransform_predictions()` was wrong, and
maybe depend on
...
mve_small_again <- multiview_embedding(data = simulated_small_4,
                                       ...
                                       lags = lags_use_multi)
```
That runs until errors, but seems to loop through everything. HERE Debug
next. BUT, results shouldn't be too great, given the short data set and short
lags.
...
HERE HERE HERE That first rho is only 0.056, presumably worse than the top ones. BUT last one
is wrong as has -ve ones that surely can't be good, and doesn't have the best
from `sort(mve_full_shebang_2$rho_each_subset)`.
`. Check calculations in
function. TODO TODO


HERE HERE HERE - could do with a function to put the list of (top) lags into a simple table.




TODO Maybe. function for size of subset_lags, just needs to be based on total number of
