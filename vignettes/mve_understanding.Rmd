...
```{r again}
set.seed(42)
# TODO have realised the correlation issue betwen S_t and R_t so modifying code
...
# Error in untransform_predictions(N_observed = response_observed, Y_predicted = response_s_predicted,  :
#  !any(is.na(Y_predicted[(min_t_Y_predicted):length(Y_predicted)])) is not TRUE
...
#. Then redo above sve_this_failed - error because lengths
             # don't match, but I think I hadn't considered all options.
...
sve_use_multi <- single_view_embedding_for_sve(
  ...
  response = "R_t",
  lags = lags_use_multi,
  max_allowed_correlation = 0.99999999)


mve_small <- multiview_embedding(data = simulated_small_3,
                                 response = "R_t",
                                 lags = lags_use_multi)
                                 # TODO (maybe ignore) orig function: when I had this as lags_use,
                                 # first six non-NA forecast values were
                                 # same as for lags_use_multi, but not the rest;
