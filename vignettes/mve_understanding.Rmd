...
# Now fixed, this helped figure it out:
...
# state_space_distances_for_sve() and dropping it from calculations. Done.
...
NEXT HERE: Need to untransform, working on `untransform_predictions()`
updated to un-first difference also. Can testcheck it works for original.

```{r untransform}
N_observed <- simulated[1:15, ]$"R_t"
N_predicted <- untransform_predictions(N_observed = N_observed,
                                       Y_predicted = response_s_predicted,
                                       max_lag = max(unlist(lags_use_multi,
                                                            use.names = FALSE)))
cor(N_observed, N_predicted[-length(N_predicted)], use =
                                                     "pairwise.complete.obs")  # Also need to remove first non-NA one
# Very low, but only a few points to get things working
```
...
TODO Can prob delete un-needed in `state_space_forecasts_for_sve.R`
