...
# Just time, observations Y_t, and predictions (my.pred is Andy's manual
# calculations):
...
results from `rEDM::Simplex()`. Andy's and Luke's code was written independently (although
Andy had explained why his results differed to those from `rEDM`).
...
Y95est_rEDM
```
as already noted above. Unlike earlier versions of `rEDM::simplex()`, the full
...
```
That does not cause an error, so this shows that changing $Y_{95}$ gives the
...
Commenting out `test_that()` in these, because it creates error -- writing the
results manually as comments.

First try setting `exclusionRadius = 1`:
```{r exclusion1}
# TODO do here then do in data-raw and save
...
# exclR = 2 has two new differences from 0
...
```
The three highlighed ones are new differences this for `exclusionRadius=3`,
