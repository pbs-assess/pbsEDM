...
independently by Andy Edwards. <!-- See our manuscript for a full explanation. -->
...
as `Y_t`, with lagged values $Y_{t-1}$ as `Y_tmin1` and $Y_{t-2}$ as `Y_tmin2`:
...
package with embedding dimension 2 (see below) as `rEDM.pred` for the predicted value and `rEDM.var`
