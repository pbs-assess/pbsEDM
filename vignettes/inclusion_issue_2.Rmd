...
Only saved data object that changed in `Nx_lags_orig2.R` is `NY_lags_example_2`
so here replace `NY_lags_example` with `NY_lags_example_2` but can leave the
...
The tibble `NY_lags_example_2` is saved in the package and contains a simple
...
function `EDM_pred_E_2()` (saved as `my.pred` and `my.var`). See `?NY_lags_example_2` for full details.
...
```{r fullresults}
as.data.frame(head(NY_lags_example_2))
as.data.frame(tail(NY_lags_example_2))
```

Note that (TODO: check this):
```{r}
NY_lags_example_2[100, c("rEDM.pred", "my.pred")]
```
...
#```{r, eval=FALSE}
 for(i in 1:nrow(NY_lags_example_2))
  {
    plotPanelMovie.df2(end = i,
                       max_time = max(NY_lags_example_2$t),
...
plot(NY_lags_example_2$rEDM.pred,
     NY_lags_example_2$my.pred,
...
different <- dplyr::filter(NY_lags_example_2,
                           abs(pred.diff) > eps)
...
This shows that eight points disagree between Andy's calculations and
rEDM's by more than $`r eps`$. (Originally there were a few more for other
reasons, but these have been corrected in rEDM). This is more than the original
`rEDM` calculations, and than those in vignette `..._3.Rmd` that uses
`rEDM::Simplex()` not `rEDM::simplex()`. The points are:
```{r}
...
```{r pbsEDMcalc}
pbs_calc <- pbsEDM(NY_lags_example_2,
                   lags = list(Y_t = c(0:1))) # A tibble (contains lists)

testthat::expect_equal(NY_lags_example_2$Y_t,
...
```{r}
plot(NY_lags_example_2$my.pred,
     pbs_pred,
...
# max(abs(NY_lags_example_2$my.pred - pbs_pred), na.rm = TRUE)
testthat::expect_equal(NY_lags_example_2$my.pred,
                       pbs_pred)
```
