...
TODO: Has been updated with new notation, still needs a careful read through and
thinking about.
...
The issue is that `rEDM` seems to use $\bf{x}_{t^*+2}$ as a nearest neighbour,
where, extending Deyle et al.'s (2013) notation
$\bf{x}_t$ is the vector of lagged values of the
scalars $Y_t$, where time $t = 1, 2, 3, ..., 99$ in this case, and.
...
which contains $Y_{95}$. But $Y_{95}$ is one of the components of
$$\bf{x}_{95} = (Y_{95}, Y_{94})$$
which is what we are trying to forecast from $\bf{x}_{94}$.
