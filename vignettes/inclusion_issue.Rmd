...
The tibble `NY_lags_example` is saved in the package and contains a simple time
...
as `Y_t`, with lagged values $X(t-1)$ as `Y_tmin1` and $X(t-2)$ as `Y_tmin2`. Also
...
```{r, testing}
testthat::expect_equal(NY_lags_example$my.pred, pbs_pred)
```
