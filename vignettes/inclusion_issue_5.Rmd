...
testthat::expect_equal(NY_lags_example_3$rEDM.pred,
                       check_rEDM)    # If no error then they match
```
...
```
which are the predicted values of $Y_{100}$. such that `rEDM` does not predict $Y_{100} = N_{101} - N_{100}$, but that is the
...
Need to re-render documentation, as I have old one.
testthat::expect_equal(NY_lags_example_3$Y_t,
                       pbs_calc2$X_observed[-length(pbs_calc2$X_observed)])
                       # verifying the indexing is the same because X_observed has an extra NA value
                       # to match pbs_calc$X_forecast (to allow for forecasts of unlagged data)
