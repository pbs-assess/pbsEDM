...
occasionally `rEDM` includes some points that should be excluded.
...
nearest neighbour, and so Aspect 2
is shown to occur for `rEDM`.

As a further check, we test the new `exclusion_radius` option with a value of 0,
...
So this is a useful test for our code with `exclusion_radius = 0`.
...
$\bf{x}_{t^*+1}$ for $t^*=75$), and
so Aspect 2 is occurring.
...
improved fit from accommodating Aspect 2 (in `pbsEDM`). So accuracy of `rEDM`
can be improved by accommodating Aspect 2 (this not guaranteed, but these
results show that it will not necessarily give worse results).
...
Also try `exclusion_radius = 7` as a further check, comparing with `rEDM` calculations:
```{r excl7}
...
Aspect 2 was originally figured out using other code (written by Andrew
