...
Note that `NY_lags_example_3` already contains results. At the end we verifty
that results using `rEDM` or `pbsEDM` have not changed from the saved ones (for
example due to package updates). TODO (maybe)
...
epsilon = 0.00001      # Small value for how different the predictions can be
...
So there are three time values for which results differ between `pbsEDM` and
`rEDM`, corresponding to $t^* =$ `r different$t-1`.

## Explaining the issue -- Aspect 2

As defined in the manuscript, vector $\bf{x}_t$ has components consisting
of the lagged values of scalars $Y_t$. For $E=2$ we have
$$\bf{x}_t = [Y_t, ~Y_{t-1}].$$
...
the nearest neighbour of $\bf{x}_{94}$, we have shown that `pbsEDM` was
originally being considered by `pbsEDM` as a nearest neighbour. But as explained
in condition (d), this should not occur.


### Calculations for $t^*=75$

We can repeat the above calculations for $t^*=75$. Skipping some of the
...
$\bf{x}_{77}$ as a nearest neighbour for $\bf{x}_{75}$, and see if get the same result as
for `pbsEDM`.
