...
plot_mve_spaces <- function(res_mve,
  ...
  tib <- make_mve_spaces_tibble(res_mve,
                         ...)
  # Make a plot, maybe using ggplot2::geom_tile(), that indicates on the y-axis
  # each value of `tib$rho` (these can be treated as factors, we just want to see
  # the number), and then x-axis gives the remaining `names(tib)`, and colours in
  # the respective box with colour `col` if that value is `TRUE` in `tib`, and
  # white if FALSE.
  ...
  # Create mapping of row_id to rounded rho values for y-axis labels
  rho_labels <- tib %>%
    dplyr::mutate(row_id = dplyr::row_number(),
  ...
  p <- ggplot2::ggplot(tib_long,
   ...
   ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 0, vjust = 0.2))
